...
def parse_pipe(pipe_str: Optional[str]) -> list[str]:
    ...
    for pipe in pipes:
        if pipe not in PIPE_FUNCS_NAMES:
            msg = f"{pipe}: please set from {PIPE_FUNCS_NAMES}"
            raise KeyError(msg)
    return pipes
...
def parse_sp(spm: Optional[str]) -> str:
    ...
    if spm not in sp_replase_dict:
        msg = f"please set from {','.join(sp_replase_dict)}"
        raise KeyError(msg)
...
def _get_argparser() -> ArgumentParser:
    ...
    parser.add_argument("-m", "--space-marker", type=parse_sp,
                        default="zenkaku", help="スペースに何を使うか")
...
def get_args_and_options(arg_str: Optional[list[str]]=None) -> tuple[Namespace, YamlDict]:
    ...
    options = YamlDict(init={
        ...
        "temporary_file": args.temporary_file
    })
    return args, options
...
def main() -> None:
    ...
    runner = RunnerPipeline(
        _bd=bobj, _ud=uobj, pipe=args.pipeline, options=options
    )
    runner.do_pipeline()
    oud = runner.get_ud()
    assert options.get("space_marker") == oud.get_sp()
    oud.write_ud_file(args.writer)
