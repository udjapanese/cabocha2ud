...
TARGET_DEP_RULE: Optional[List] = None
DEP_RULE_FILE: dict[str, str] = {
    # word unit mapping
...
def check_funcname(func_name: str, rule_set) -> List[str]:
    """
...
def load_dep_rule(data_type: str, word_unit: str) -> List:
    """
        load rule file
    """
    yaml = ruamel.yaml.YAML()
    rule_set: dict = yaml.load(open(DEP_RULE_FILE[data_type + "_" + word_unit]).read().replace('\t', '    '))
